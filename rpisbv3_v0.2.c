...
{
  PORTD &= ~0xFC;  //clear all but three lowest.
  PORTC &= ~0x03;  //and lowest two in C.
}
...
ISR (TIMER2_COMP_vect)
{
  ...
  OCR1A = servo[current_servo] - 17;  // -17 is for interrupt time.
...
//PORTS
  ...
  DDRC =  0x03;          // Port Pin 0 and 1 is servo out.
  DDRC |= 0x08;          // Port Pin 3 is 741 disable.
  DDRC |= 0x30;          // Port Pin 4 and 5 output for pin 2 and 3 of extra connector.
  
  DDRB =  0x3C;          // Port Pin 2..5 output for pin 4..7 of extra connector.
  PORTB = 0x00;          // 
